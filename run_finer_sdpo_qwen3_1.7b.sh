#!/usr/bin/env bash
# FiNER SDPO with Qwen3-1.7B (smaller-model baseline for the planned co-SDPO
# experiment) on 4x A6000 (48 GB each).
#
# Identical recipe to run_finer_sdpo_full_v2.sh except:
#   - actor_rollout_ref.model.path: Qwen/Qwen3-4B-Thinking-2507 -> Qwen/Qwen3-1.7B
#   - exp_name suffix updated
# Everything else (batches, response cap, KL coef, GEPA/context_updater off,
# rollout_n, val_kwargs, etc.) is held constant so the per-step numbers are
# directly comparable to v2.
#
# Expected: peak GPU mem drops from ~41 GB -> ~18-22 GB; step time drops from
# ~5 min -> ~2-3 min; full run ~5-6 h on 4x A6000.
...
exp_name='qwen3_1.7b_full_run_resp12288'

...
class DataParallelPPOActor(BasePPOActor):
    ...
    @staticmethod
    def _resolve_self_distillation_cfg(raw_cfg):
        """Convert a plain dict to SelfDistillationConfig if needed.

        Ray serialization can convert OmegaConf DictConfig to a plain dict,
        which breaks attribute access. This method ensures the config is always
        a proper SelfDistillationConfig dataclass.
        """
        if raw_cfg is None:
            return None
        if isinstance(raw_cfg, dict):
            from ...workers.config.actor import SelfDistillationConfig

            return SelfDistillationConfig(**raw_cfg)
        return raw_cfg
    ...
    def _forward_micro_batch(
        ...
        """
        Args:
            return_all_logps: (SDPO) If True, return log-softmax over the full vocabulary for every
        ...
        Returns:
            ...
            Conditionally contains:
                entropys: (bs, response_len) — if calculate_entropy is True
                sum_pi_squared: (bs, response_len) — if calculate_sum_pi_squared is True
                all_logps: (bs, response_len, vocab_size) — if return_all_logps is True and use_topk is False
                topk_logps: (bs, response_len, k) — if distill_topk or topk_indices is set
                topk_indices: (bs, response_len, k) — if distill_topk is set and topk_indices is None
        """
        ...
        if (return_all_logps or use_topk) and self.use_fused_kernels:
            raise ValueError("Logit distillation requires disabling fused kernels, because with fused kernels, the model internally computes log probs and entropy inside the fused kernel and returns them directly.")
    ...
    def compute_log_prob(self, data: DataProto, calculate_entropy: bool = False) -> dict[str, torch.Tensor]:
        ...
        has_multi_modal_inputs = (
            ...
            and any(len(m) > 0 for m in data.non_tensor_batch["multi_modal_inputs"])
        )

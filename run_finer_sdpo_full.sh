#!/usr/bin/env bash
# Full FiNER SDPO run with Qwen3-4B-Thinking-2507 on 4x A6000 (48GB each).
#
# Differences vs run_smoke_finer.sh:
#   - no total_training_steps cap (1 epoch = 125 outer batches)
#   - val_before_train True; test_freq 25 (~5 evals over 1 epoch)
#   - save_freq 25, max_actor_ckpt_to_keep 1 (one rolling checkpoint)
#   - dump rollouts/reprompts/val generations to checkpoints/ for inspection
#   - logger: console (override to add wandb if WANDB_API_KEY is set)
...
exp_name='qwen3_4b_full_run'

# Default to console logger; override via env if WANDB is available

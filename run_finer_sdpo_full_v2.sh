#!/usr/bin/env bash
# Full FiNER SDPO run v2 with longer response budget on 4x A6000 (48 GB each).
#
# Differences vs run_finer_sdpo_full.sh:
#   - max_response_length: 4096 -> 12288 (was hitting 0.91 clip_ratio at 4096)
#   - ppo_max_token_len_per_gpu: 12288 -> 16384 (FSDP forward/backward budget)
#   - rollout.max_model_len: 12288 -> 16384 (vLLM KV cache budget)
#   - rollout.max_num_batched_tokens: defaults handled, raise via max_model_len
#   - max_reprompt_len: 8192 -> 16384 (so teacher reprompt context isn't truncated)
#   - exp_name suffix bumped to avoid colliding with v1 checkpoints
#
# Risk: each step ~2-3x slower (-> ~20-25 h wall clock); peak GPU mem may
# climb from ~31 GB to ~40-45 GB. Still inside 48 GB envelope.
...
project_name='sdpo_finer_a6000'
exp_name='qwen3_4b_full_run_v2_resp12288'
